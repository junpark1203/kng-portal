    ...
    if (exportDataStr) {
        try {
            ...
            if (exportData && exportData.length > 0) {
                ...
                openItemModal();
                document.getElementById('itemItemName').value = title;
                
                // Populate variants
                exportData.forEach(item => {
                    const specStr = `[${item.identifier}] ${item.name} ${item.specification ? '('+item.specification+')' : ''} - 수량:${item.totalQuantity} (${item.sites.join(', ')})`;
                    // We need to wait a tiny bit for the DOM to be ready inside openItemModal
                    setTimeout(() => addInitialVariantRow(specStr, item.unit || 'EA'), 50);
                });
...
async function saveItem(status = 'completed') {
