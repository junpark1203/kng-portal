    ...
    db.serialize(() => {
        ...
        `, (err) => {
            if (err) console.error('products 테이블 생성 오류:', err.message);
                ...
                db.run(`ALTER TABLE products ADD COLUMN status TEXT DEFAULT 'active'`, (alterErr) => {
                    if (alterErr && !alterErr.message.includes('duplicate column')) {
                        console.error('products ALTER TABLE (status) 오류:', alterErr.message);
                    }
                });
            }
        });
...
app.post('/api/products', (req, res) => {
    ...
    const sql = `INSERT INTO products (id, date, mcode, catEn, catKo, nameEn, nameKo, priceKrw, domesticShipping, packagingKrw, rate, rateDate, weight, link, note, description, notice, createdAt, updatedAt, images, video, optionName, parentImages, descKo, status)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
    const params = [id, p.date||'', p.mcode||'', p.catEn||'', p.catKo||'', p.nameEn||'', p.nameKo||'',
                    p.priceKrw||0, p.domesticShipping!=null?p.domesticShipping:3000, p.packagingKrw||0, p.rate||0, p.rateDate||'', p.weight||0, p.link||'', p.note||'', p.description||'', p.notice||'', now, now, p.images||'[]', p.video||'', p.optionName||'', p.parentImages||'[]', p.descKo||'', p.status||'active'];
...
app.put('/api/products/:id', (req, res) => {
    ...
    const p = req.body;
    const now = new Date().toISOString();
    const sql = `UPDATE products SET date=?, mcode=?, catEn=?, catKo=?, nameEn=?, nameKo=?, priceKrw=?, domesticShipping=?, packagingKrw=?, rate=?, rateDate=?, weight=?, link=?, note=?, description=?, notice=?, updatedAt=?, images=?, video=?, optionName=?, parentImages=?, descKo=?, status=?
                 WHERE id=?`;
    const params = [p.date||'', p.mcode||'', p.catEn||'', p.catKo||'', p.nameEn||'', p.nameKo||'',
                    p.priceKrw||0, p.domesticShipping!=null?p.domesticShipping:3000, p.packagingKrw||0, p.rate||0, p.rateDate||'', p.weight||0, p.link||'', p.note||'', p.description||'', p.notice||'', now, p.images||'[]', p.video||'', p.optionName||'', p.parentImages||'[]', p.descKo||'', p.status||'active', id];
    db.run(sql, params, function(err) {
        if (err) return res.status(500).json({ error: err.message });
